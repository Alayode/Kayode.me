 ...
 * @since 1.0.0
 */
function themify_theme_video_meta_box( $args = array() ) {
	extract( $args );
	return array(
		// Layout
		array(
...
/**************************************************************************************************
 * Video Class - Shortcode
 **************************************************************************************************/

if ( ! class_exists( 'Themify_Video' ) ) {
	...
	class Themify_Video {
		...
		/**
		 * Set during video loop execution to store the featured image.
		 * Used to extract the URL to the image for the video slider pagination.
		 * @var string
		 */
		var $video_image = '';
		/**
		...
		function __construct( $args = array() ) {
			...
			add_filter( 'wp_video_shortcode', array( $this, 'wp_video_shortcode' ) );
		...
		function shortcode($atts = array(), $post_type) {
			...
			if ($posts) {
				...
				$video_loop_id = $is_slider ? "$post_type-slider-$this->instance" : "$post_type-loop-$this->instance";
				$out = "<div id='$video_loop_id' class='loops-wrapper shortcode $post_type $style $cpt_layout_class'>$loop</div>";
		...
		/**
		 * Fetch the video thumbnail from different services
		 * @param bool $make_tag
		 * @param string $width
		 * @return string
		 */
		function fetch_video_image( $make_tag = true, $width = '' ) {
			$image_url = '';
			$video_url = '';
			if ( themify_get( 'video_url' ) != '' ) {
				$video_url = themify_get( 'video_url' );
			} elseif ( themify_get( 'lightbox_link' ) != '' ) {
				$video_url = themify_get( 'lightbox_link' );
			} elseif ( themify_get( 'external_link' ) != '' ) {
				$video_url = themify_get( 'external_link' );
			}
			if ( preg_match( '%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $video_url, $match ) ) {
				...
				$response_body = wp_remote_retrieve_body( $request );
				if ( '' != $response_body ) {
					$fod = json_decode( $response_body );
					$image_url = $fod->thumbnail_url;
				}
				...
				if ( ! is_wp_error( $request ) ) {
					...
					if ( isset( $data ) && isset( $data->thumbnail_large_url ) ) {
						$image_url = $data->thumbnail_large_url;
					}
				} else {
					$image_url = 'http://www.dailymotion.com/thumbnail' . str_replace( '/embed', '', $video_id );
				}
			} elseif( false !== stripos( $video_url, 'blip' ) ) {
				$request = wp_remote_get( 'http://blip.tv/oembed?url=' . $video_url, array( 'sslverify' => false ) );
				if ( ! is_wp_error( $request ) ) {
					$data = json_decode( wp_remote_retrieve_body( $request ) );
					if ( isset( $data ) && isset( $data->thumbnail_url ) ) {
			...
			if ( $make_tag ) {
				return '<img src="' . themify_https_esc( $image_url ) . '" width="' . $width . '" />';
			}
			return $image_url;
		}
		...
		function get_featured_image_link( $args = array() ) {
			...
			if ( '' != themify_get( 'video_url' ) ) {
				$link = esc_url( themify_get( 'video_url' ) );
		...
		/**
		 * Replaces fixed width with elastic.
		 * @param $html
		 * @return mixed
		 */
		function wp_video_shortcode( $html ) {
			$html = preg_replace( '/(width)=".*?"/i', 'width="100%"', $html );
			$html = preg_replace( '/style="width(.*?);/i', 'style="width: 100%;', $html );
			$html = preg_replace( '/style="width(.*?)height(.*?);/i', 'style="width: 100%;', $html );
			return $html;
		}
		 ...
		 * @return string
		 */
		function get_video_type( $post_id ) {
			if ( $type = get_post_meta( $post_id, 'video_type', true ) ) {
				return $type;
			}
			return 'embed';
		}
 ...
 **************************************************************************************************/
$GLOBALS['themify_video'] = new Themify_Video();