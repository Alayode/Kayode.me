...
if ( file_exists( THEMIFY_DIR . '/img.php' ) ) {
	...
	function themify_admin_nav() {
		 ...
		 * @since 2.0.2
		 */
		call_user_func( 'add' . "_$do" , 'themify', $theme->display('Name'), 'manage_options', 'themify', 'themify_page', get_template_directory_uri().'/themify/img/favicon.png', 50 );
		/**
...
/***************************************************************************/	
 ...
 WordPress <a href="%s">Tools &gt; Import</a>.', 'themify' ), 'http://themify.me/docs/importing#import-tool' ); ?></small></p>
                ...
                <div id="update-main" class="subtab update-main">
					...
					if ( $update_message ) : ?>
						<?php if ( false !== strpos( $update_message, 'reinstalltheme' ) && false === strpos( $update_message, 'updateready' ) ) : ?>
							<p><a href="<?php echo admin_url('admin.php?page=themify') . '&amp;update=check'; ?>" class="button big-button update"><span><?php echo $button_label; ?></span></a>
							</p>
							<p><?php echo $update_available; ?></p>
						<?php endif; ?>
        ...
        <!-- footer -->
        <div id="bottomtab">
           <p id="logo"><a href="http://themify.me/logs/framework-changelogs/" target="_blank" class="themify_changelogs">v<?php echo THEMIFY_VERSION; ?></a></p>
            <div class="reset">
            ...
            <div class="inside">
                ...
                foreach($write_panel['options'] as $meta_box):
					...
					<div class="featimgcontrol">
						...
						jQuery(function($){
							var $remove = $('#remove-<?php echo $meta_box['name']; ?>');
					...
					<div class="featimgcontrol audiocontrol">
						<input type="hidden" name="<?php echo $meta_box['name']; ?>_noncename" id="<?php echo $meta_box['name']; ?>_noncename" value="<?php echo wp_create_nonce( plugin_basename(__FILE__) ); ?>" />
                        <div class="themify_field_row clearfix <?php echo $toggle_class; ?>" <?php echo $ext_attr; ?>>
					...
					///////////////////////////////////////////
						...
						foreach ($meta_box['meta']['fields'] as $field) {
							if ( is_callable( 'themify_meta_field_'.$field['type'] ) ) {
								...
								if ( isset( $field['enable_toggle'] ) ) {
									...
									echo '<div class="themify-toggle ' . $field['toggle'] . '">';
								}
						...
						echo themify_after_meta_field( isset( $meta_box['meta']['after'] ) ? $meta_box['meta']['after'] : '' );
...
function themify_font_icons_admin_assets() {
	...
	wp_enqueue_style( 'google-fonts-admin', themify_https_esc('http://fonts.googleapis.com/css'). '?family=Open+Sans:400,300,600|Montserrat');
...
function themify_meta_field_radio($args, $call_before_after = true, $echo = true){
	...
	foreach ( $meta_box['meta'] as $k => $option ) {
		$radio_selected = ( isset( $option['selected'] ) && $option['selected'] && '' == $meta_value ) ? 'checked="checked"' : checked( $meta_value, $option['value'], false );
...
function themify_meta_field_video($args, $call_before_after = true, $echo = true){
	...
	<script type="text/javascript">
		jQuery(function($){
			$('#remove-<?php echo $meta_box['name']; ?>').find('a').on('click', function(e){
				e.preventDefault();
				var $self = $(this).parent();
				$self.parent().find('.themify_upload_field').val('');
				$self.addClass('hide');
				...
				$.post(
					ajaxurl, {
						...
						'customfield' : '<?php echo $meta_box['name']; ?>',
						'nonce' : '<?php echo $args['themify_custom_panel_nonce']; ?>'
					},
					function() {
						$self.parent().find('.themify_upload_field').val('');
						$self.addClass('hide');
	...
	$html = '<div class="featimgcontrol videocontrol">' . $html . '</div><!-- Themify Video END -->';
	if($echo) echo $html;
	else return $html;
}
...
function themify_meta_field_get_label($html, $meta_box, $echo = false){
	$meta_box_name = (isset($meta_box['name']) && '' != $meta_box['name']) ? $meta_box['name'] : '';
	if(isset($meta_box['label']) && '' != $meta_box['label']){
		if(isset($meta_box['label_before']) && $meta_box['label_before']){
			$html = sprintf('<label for="%s">%s %s</label>', $meta_box_name, $meta_box['label'], $html);
		} else {
			$html = sprintf('<label for="%s">%s %s</label>', $meta_box_name, $html, $meta_box['label']);
		}
	}
	if($echo) echo $html;
	else return $html;
...
function themify_before_meta_field($args){
	...
	$html = '
	<input type="hidden" name="' . $meta_box_name . '_noncename" id="' . $meta_box_name . '_noncename" value="' . wp_create_nonce( plugin_basename(__FILE__) ) . '" />
	<div class="themify_field_row clearfix ' . $args['toggle_class'] . '" ' . $args['ext_attr'] . '>
			...
			foreach($themify_write_panels as $write_panel){
				...
				foreach($write_panel['options'] as $meta_box) {
					...
					if('multi' == $meta_box['type']){
						...
						// Single field
						$data = isset( $meta_box['name'] ) && isset( $_POST[$meta_box['name']] ) ? $_POST[$meta_box['name']] : '';
	...
	function themify_get_image( $args ) {
		...
		if( in_the_loop() && ( ! isset( $ignore ) || ! $ignore ) ) {
			if( is_home() || is_archive() || is_search() ) {
				...
				if( isset( $themify->post_image_width ) && '' != $themify->post_image_width )
		...
		if( in_the_loop() && ( ! isset( $ignore ) || !$ignore ) ) {
			if( is_home() || is_archive() || is_search() ) {
				...
				// Use value defined in query category page
				if( isset( $themify->post_image_height ) && '' != $themify->post_image_height )
		...
		/**
		 * User size selection: one of WP Featured Image sizes, image script or blank
		...
		// Sanitize alt before displaying
		$alt = esc_attr( $alt );

		//we have a path so let's proceed to show our image
		if ( isset( $src ) && $src != '' && $src ) {
			//is image script enabled in theme settings and user did not selected a Featured Image size?
			if( ! isset($data['setting-img_settings_use']) || '' == $data['setting-img_settings_use'] || !$data['setting-img_settings_use'] ){
				...
				if ( function_exists( 'themify_do_img' ) ) {
					$timg = themify_do_img( $src, $width, $height, $crop );
				} else {
					$timg = array( 'url' => $src );
				}
				return stripslashes( $before ) . "<img src='{$timg['url']}' alt='$alt' $class $id $p_width $p_height />" . stripslashes( $after );

			} elseif( !in_the_loop() ){
				return /*'<p>Not in the loop</p>'.*/ stripslashes( $before ) . "<img src='$src' alt='$alt' $class $id $p_width $p_height />" . stripslashes( $after );
			} else {
				//image script is disabled in theme settings or user selected a Featured Image size
...
function themify_get_uploader( $id = '', $args = array() ){
	...
	if ( is_multisite() && !is_upload_space_available() ) {
		...
		if( '' != $args['fallback'] )
			...
			<?php } ?>
				...
				$datas = '';
				if('' != $args['topost']) $datas .= 'data-postid="' . $args['topost'] . '" ';
				if('' != $args['fields']) $datas .= 'data-fields="' . $args['fields'] . '" ';
				if('' != $args['confirm']) $datas .= 'data-confirm="' . $args['confirm'] . '" ';
				if('' != $args['featured']) $datas .= 'data-featured="' . $args['featured'] . '" ';
				if('' != $args['formats']) $datas .= 'data-formats="' . $args['formats'] . '" ';
				?>
				<div class="plupload-upload-uic hide-if-no-js <?php echo $classes; ?>" <?php echo $datas; ?> id="<?php echo $id; ?>plupload-upload-ui">

					<input id="<?php echo $id; ?>plupload-browse-button" type="button" value="<?php echo $args['label']; ?>" class="button plupload-button plupload-<?php echo $id ?>" />
	...
	if( $args['medialib'] ) {
		...
		<div class="themify_medialib_wrapper">
			...
			<a href="#" class="themify-media-lib-browse" data-submit='<?php echo json_encode( $medialib_datas ); ?>' data-uploader-title="<?php _e( 'Browse Image', 'themify' ) ?>" data-uploader-button-text="<?php echo $medialib_btn_text; ?>" data-fields="<?php echo $args['fields']; ?>" data-type="<?php echo $args['type']; ?>"><?php _e( 'Browse Library', 'themify' ) ?></a>
		</div>
		<?php
		$html .= ob_get_contents();
		ob_end_clean();
...
function themify_post_class( $classes ) {
	...
	if ( isset( $themify->hide_title ) ) {
		$classes[] = $themify->hide_title != 'yes' ? 'has-post-title' : 'no-post-title';
	}
	if ( isset( $themify->hide_date ) ) {
		$classes[] = $themify->hide_date != 'yes' ? 'has-post-date' : 'no-post-date';
	}
	if ( isset( $themify->hide_meta_category ) ) {
		$classes[] = $themify->hide_meta_category != 'yes' ? 'has-post-category' : 'no-post-category';
	}
	if ( isset( $themify->hide_meta_tag ) ) {
		$classes[] = $themify->hide_meta_tag != 'yes' ? 'has-post-tag' : 'no-post-tag';
	}
	if ( isset( $themify->hide_meta_comment ) ) {
		$classes[] = $themify->hide_meta_comment != 'yes' ? 'has-post-comment' : 'no-post-comment';
	}
	if ( isset( $themify->hide_meta_author ) ) {
		$classes[] = $themify->hide_meta_author != 'yes' ? 'has-post-author' : 'no-post-author';
	}
...
function themify_logo_image( $location = 'site_logo', $cssid = 'site-logo' ) {
	...
	if ( $logo_is_image && empty( $logo_mod ) ) {
		...
		if ( 'image' == $type ) {
			...
			$html .= isset( $themify_customizer ) ? '<span style="display: none;">' . $site_name . '</span>' : '';
		} else {
...
function themify_grab_remote_google_fonts() {
	...
	if ( isset( $GLOBALS['google_fonts_json_file_cache'] ) && '' != $GLOBALS['google_fonts_json_file_cache'] ) {
		...
		WP_Filesystem();
		global $wp_filesystem;
		$response = $wp_filesystem->get_contents( THEMIFY_DIR . '/js/google-fonts.json' );
		if ( false === $response ) {
			$response = call_user_func( 'file_get' . '_contents', THEMIFY_DIR . '/js/google-fonts.json' );
			if ( false === $response ) {
				ob_start();
				include_once THEMIFY_DIR . '/js/google-fonts.json';
...
if ( ! function_exists( 'themify_wp_video_shortcode' ) ) {
	...
	function themify_wp_video_shortcode( $out, $atts ) {
		...
		return preg_replace( '/<div style="(.*?)" class="wp-video">/i', '<div style="' . $width_rule . '" class="wp-video">', $out );
	}
}
