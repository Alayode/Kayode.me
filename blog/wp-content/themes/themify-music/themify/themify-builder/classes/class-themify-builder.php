...
if ( ! class_exists( 'Themify_Builder' ) ) {
	...
	class Themify_Builder {
		...
		public function builder_cpt_check() {
			...
			if( ! is_array( $post_types ) ) {
				...
				foreach( array( 'slider', 'highlight', 'testimonial', 'portfolio' ) as $post_type ) {
					$count = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->posts} WHERE post_type = '{$post_type}'" );
					if( $count > 0 ) {
		...
		function load_front_js_css() {
			...
			// Setup Animation
			$inview_selectors = apply_filters( 'themify_builder_animation_inview_selectors', array() );
			$new_selectors = apply_filters( 'themify_builder_create_animation_selectors', array() );
			
			$global_selectors = isset( $new_selectors['selectors'] ) ? $new_selectors['selectors'] : array();
			$specific_selectors = isset( $new_selectors['specificSelectors'] ) ? array_keys( $new_selectors['specificSelectors']) : array();
			$instyle_selectors = array_merge( $inview_selectors, $global_selectors, $specific_selectors );
			...
			wp_localize_script( 'themify-builder-script-js', 'tbLocalScript', array( 
				...
				'animationInviewSelectors' => $inview_selectors,
				'createAnimationSelectors' => $new_selectors,
		...
		function module_lightbox_options_ajaxify() {
			...
			if ( false !== $module ) {
				...
				echo '<p>' . sprintf( __( 'Module %s is not active', 'themify' ), $module_name ) . '</p>';
			}
		...
		function module_edit_panel_front( $mod_name, $mod_settings ) {
			...
			<div class="module_menu_front">
				...
				<?php
					$mod_settings = json_encode( $this->clean_json_bad_escaped_char( $mod_settings ) );
				?>
				<div class="front_mod_settings mod_settings_<?php echo $mod_name; ?>" data-mod-name="<?php echo esc_attr( $mod_name ); ?>">
					<script type="text/json"><?php echo $mod_settings; ?></script>
				</div>
			</div>
		...
		public function get_template_row( $rows, $row, $builder_id, $echo = false ) {
			...
			?>
			<!-- module_row -->
			<div data-gutter="<?php echo esc_attr( $row_gutter_class ); ?>" class="<?php echo implode( ' ', $row_classes ); ?>" <?php echo $data_video; ?>>
												...
												if ( isset( $col['modules'] ) && count( $col['modules'] ) > 0 ) { 
													foreach ( $col['modules'] as $modules => $mod ) {
														...
														if ( isset( $mod['cols'] ) && count( $mod['cols'] ) > 0 ) {
															...
															$sub_row_attr = $this->frontedit_active ? ' data-gutter="' . $sub_row_gutter . '"' : '';
